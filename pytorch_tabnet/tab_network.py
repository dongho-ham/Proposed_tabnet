...
def conv5x5(in_channels, out_channels, stride=1, padding=2, dilation=1):
    ''' 5x5 convolution '''
    return nn.Conv2d(in_channels, out_channels, kernel_size=5, stride=stride, padding=padding, dilation=dilation, bias=False)
...
class CBAM(nn.Module):
    ...
    Convolutional Block Attention Module Class
    Ref: Woo et al., "CBAM: Convolutional Block Attention Module", ECC
    2018, https://arxiv.org/abs/1807.06521

    Args:
        in_channel (int): Number of input channels
        reduction_ratio (int): Reduction ratio for channel attention
        dilation (int): Dilation rate for spatial attention
    Returns:
        Tensor after applying CBAM attention 
    """
    def __init__(self, in_channel, reduction_ratio=5, dilation=1):
        ...
        self.conv1 = conv3x3(2, 1, stride=1, dilation=self.dilation)
    ...
    def forward(self, x):
        ...
        if not self.set_mlp:
            self.mlp = nn.Sequential(
            nn.Linear(in_features=avgOut.size(-1), out_features=self.hid_channel),
            nn.ReLU(),
            nn.Linear(in_features=self.hid_channel, out_features=avgOut.size(-1))
            )
            self.set_mlp = True
        ...
        Mf2 = torch.mean(input=Mf2, dim=1)
        
        return Mf2
    
class CNN_CBAM(nn.Module):
    def __init__(self, mine, num_filters=5, kernel_size=5):
        """
        ...
        ----------
        F : int
            Number of features in the input and output
        num_filters : int
        ...
        # Assuming the same number of filters and kernel size for all conv layers
        self.conv1 = nn.Conv1d(mine, num_filters, kernel_size, padding=kernel_size//2)
        self.conv2 = nn.Conv1d(num_filters, num_filters, kernel_size, padding=kernel_size//2)
        self.conv3 = nn.Conv1d(num_filters, mine, kernel_size, padding=kernel_size//2)
        
        #CBACM
        self.cbam1 = CBAM(in_channel=num_filters)
        self.cbam2 = CBAM(in_channel=num_filters)
        
        # Fully connected layer to match the output shape (N, F)
        self.fc = nn.Linear(mine, mine)

    def forward(self, x):
        #print(x.shape)
        # Reshape input to (N, C, L)
        x = x.unsqueeze(2)  # Assuming input x is of shape (N, F)
        # Apply three Conv1D layers
...
class AttentiveTransformer(torch.nn.Module):
    def __init__(
        ...
        self.cbam_module = CNN_CBAM(mine=group_dim)
        #print("Custom AttentiveTransformer is loaded")
    ...
    def forward(self, priors, processed_feat):
        #print("Custom AttentiveTransformer is working")
        x = self.fc(processed_feat)
        x = self.bn(x)
        priors = self.cbam_module(priors)
